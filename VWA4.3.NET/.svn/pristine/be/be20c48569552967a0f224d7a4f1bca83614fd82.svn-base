    ...
    public partial class rptComparision : DataDynamics.ActiveReports.ActiveReport
    {
        ...
        private void rptComparision_ReportStart(object sender, EventArgs e)
        {
            ...
            if (_InputParameters["Filter"].ParamValue != "")
                where += "(" + _InputParameters["Filter"].ParamValue + ") AND";
            ...
            if (_InputParameters["ComparisionType"].DisplayValue == "Days of Week")
            {
                select = @"SELECT Name, Waste, Waste2 FROM(" + 
                    ...
                    " (Timestamp >= #" + DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).ToString("yyyy/MM/dd") + "# AND Timestamp < #" +
                    DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).AddDays(7).ToString("yyyy/MM/dd") + "#) " +
                    " UNION     " +
                    ...
                    where +
                    " (Weights.Timestamp >= #" + DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).ToString("yyyy/MM/dd") + "# AND Weights.Timestamp < #" +
                    DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).AddDays(7).ToString("yyyy/MM/dd") + "#) " +
                    ...
                    " (SELECT DISTINCT Weekday(Timestamp) AS Name, Format(Timestamp,'yyyymmdd'), 0.0001 AS WasteValue  " +
                    " FROM WeightDates  WHERE " +
                    " (Timestamp >= #" + DateTime.Parse(_InputParameters["SecondWeekStart"].ParamValue).ToString("yyyy/MM/dd") + "# AND Timestamp < #" +
                    DateTime.Parse(_InputParameters["SecondWeekStart"].ParamValue).AddDays(7).ToString("yyyy/MM/dd") + "#) " +
                    " UNION  " +
                    " SELECT Weekday(Weights.Timestamp) AS Name, Format(Weights.Timestamp,'yyyymmdd'), " + criteria + " AS WasteValue  " +
                    " FROM ((Weights LEFT JOIN Transfers ON Transfers.TransKey = Weights.TransKey) LEFT JOIN LossType ON Weights.LossTypeID = LossType.TypeID)   " +
                    (isWasteClassesUsed ? " LEFT JOIN FoodType ON Weights.FoodTypeID = FoodType.TypeID " : "") +
                    where +
                    " (Weights.Timestamp >= #" + DateTime.Parse(_InputParameters["SecondWeekStart"].ParamValue).ToString("yyyy/MM/dd") + 
            ...
            else
            {
                ...
                select = @"SELECT Name, Waste, Waste2 FROM(" +
                    ...
                    where +
                    " (Weights.Timestamp >= #" + DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).ToString("yyyy/MM/dd") + 
                    "# AND Weights.Timestamp < #" +
                    DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue).AddDays(7).ToString("yyyy/MM/dd") + "#) " +
            ...
            if (_ChartData.Rows.Count > 0)
            {
                ...
                DataTable total = VWA4Common.DB.Retrieve(@"SELECT " + criteria + " FROM Weights LEFT JOIN Transfers ON Transfers.TransKey = Weights.TransKey " +
                    ...
                    "# AND [Weights.Timestamp] < #" + 
                    DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue.ToString()).AddDays(7).ToString("yyyy/MM/dd") + 
                ...
                string testquery = @"SELECT " + criteria + " FROM Weights LEFT JOIN Transfers ON Transfers.TransKey = Weights.TransKey " +
                    ...
                    "# AND [Weights.Timestamp] < #" + 
                    DateTime.Parse(_InputParameters["SecondWeekStart"].ParamValue.ToString()).AddDays(7).ToString("yyyy/MM/dd") + 
                    "# AND SiteID = " + _InputParameters["SiteID"].ParamValue;
                ...
                if (_InputParameters["ComparisionType"].DisplayValue == "Days of Week")
                {
                    DateTime firstDayOfWeek = DateTime.Parse(_InputParameters["FirstWeekStart"].ParamValue);
                    for (int i = 0; i < 7; i++)
