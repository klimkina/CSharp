        ...
        private void rptARTrend_ReportStart(object sender, EventArgs e)
        {
            ...
            if (_ChartData.Rows.Count > 0)
            {
                ...
                for (step = 1; temp > 1; step *= 10)
                    temp = temp / 10;
                step = step / 100 * (int)(1 + max * 10 / step);
                this.chartControl1.ChartAreas[0].Axes["AxisY2"].MajorTick.Step = step;
                ...
                if (bool.Parse(_InputParameters["IsChartOnly"].ParamValue))
                {
                    this.chartControl1.Titles["Title"].Text = "Date: " + 
                        DateTime.Parse(_InputParameters["StartDate"].ParamValue).ToString("MM/dd/yyyy") + " - " + 
                ...
                else
                {
                   ...
                   //" Total" + title_top + totalSum.ToString("0") + end;
                    ...
                    if (dtTransNum != null && dtTransNum.Rows.Count > 0 &&
                        int.Parse(dtTransNum.Rows[0][0].ToString()) <= double.Parse(_InputParameters["LowTransactionLimit"].ParamValue) * double.Parse(VWA4Common.GlobalSettings.GetBaselineWeeklyWasteTrans(_InputParameters["SiteID"].ParamValue)))
                    {
                        int weekNum;
                        ...
                        for (int i = 1; i < dtTransNum.Rows.Count; i++)
                            if (int.Parse(dtTransNum.Rows[i]["TransNum"].ToString()) <= double.Parse(_InputParameters["LowTransactionLimit"].ParamValue) * double.Parse(VWA4Common.GlobalSettings.GetBaselineWeeklyWasteTrans(_InputParameters["SiteID"].ParamValue)))
                            {
                                lblWarning.Text += ", ";
