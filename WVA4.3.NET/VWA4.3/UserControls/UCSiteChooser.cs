    ...
    public partial class UCSiteChooser : UserControl
    {
        ...
        public void EnableAllSites(bool isEnable)
        {
            ...
            if (!isEnable)
            {
                ...
                {
                    int i = cbSite.SelectedIndex;
                    cbSite.Items.RemoveAt(0);
                    if (i > 0)
                        cbSite.SelectedIndex = i - 1;
                    else
                        cbSite.SelectedIndex = _CurrSite;
                }
            }
            else
            {
                if (((VWA4Common.VWACommon.MyListBoxItem)(cbSite.Items[0])).ItemData.Split(',')[0] != "-1")
                {
                    _CurrSite = cbSite.SelectedIndex;
        ...
        public bool IsAllSites
        {
            get
            {
                return ((VWA4Common.VWACommon.MyListBoxItem)(cbSite.Items[0])).ItemData.Split(',')[0] == "-1";
            }
            set { EnableAllSites(value); }
        }
