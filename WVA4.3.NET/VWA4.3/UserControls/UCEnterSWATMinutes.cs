...
{
	public partial class UCEnterSWATMinutes : UserControl, IVWAUserControlBase
	{
		...
		{
            bSave.Enabled = bool.Parse(SecurityManager.GetSecurityManager()["Enter SWAT Notes"]);
			Initialized = false;
			// Null out fields
			...
			// Load Date Range
			//
			lDateRange.Text = DateTime.Parse(VWA4Common.GlobalSettings.StartDateOfSelectedWeek).ToString("MM/dd/yyyy")
				+ " to " + DateTime.Parse(VWA4Common.GlobalSettings.StartDateOfSelectedWeek).AddDays(6).ToString("MM/dd/yyyy");
			//
			...
			bSave.Hide();
            btnPrint.Hide();
			lforSite.Text = "(for " + VWA4Common.GlobalSettings.CurrentSiteName + ")";
			Initialized = true;
		}
		...
		{
            bSave.Enabled = bool.Parse(SecurityManager.GetSecurityManager()["Enter SWAT Notes"]);
			if (this.IsActive && !e.IsLogin) // ||  !bool.Parse(SecurityManager.GetSecurityManager().GetDBManagerPermission("Enter SWAT Notes")))
				commonEvents.TaskSheetKey = "dashboard";
		}
