...
namespace Reports
{
    ...
    public partial class rptEmployeeTransactions : DataDynamics.ActiveReports.ActiveReport
    {
        ...
        private void rptEmployeeTransactions_ReportStart(object sender, EventArgs e)
        {
            ...
            txtPeriod.Text = start.ToString("MM/dd/yyyy") + " - " + 
                start.AddDays(7 * int.Parse(_InputParameters["NumberOfWeeks"].ParamValue)).ToString("MM/dd/yyyy");
            ...
            string from = @" FROM (SELECT UserTypeID, TypeName, MAX(res0) AS result0 " +
                    ...
                    " Weights.Timestamp >= #" + start.ToString("yyyy/MM/dd") + "# AND Weights.Timestamp < #" + start.AddDays(7).ToString("yyyy/MM/dd") + "# " +
